...
class TurtleSpawnerNode(Node):
    ...
    def spawn_new_turtle(self):
        self.turtle_conter_ += 1
        name = self.turtle_name_prefix_ + str(self.turtle_conter_)
    ...
    def callback_call_spawn(self, future: Future, turtle_name, x, y, theta):
        try:
            response: Spawn.Response = future.result()
            if response.name != "":
                self.get_logger().info(f"Turtle {turtle_name} is now alive")
    ...
    def callback_call_kill(self, future: Future, turtle_name):
        try:
            future.result()
