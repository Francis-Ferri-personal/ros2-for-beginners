...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.pose_subscriber = self.create_subscription(
            Pose, "turtle1/pose", self.callback_tutle_pose, 10
        )
        ...
        self.control_llop_timer = self.create_timer(0.01, self.control_loop)

    def callback_tutle_pose(self, msg: Pose):
        self.pose_ = msg

    def callback_alive_turtles(self, msg: TurtleArray):
        if len(msg.turtles) > 0:
            if self.catch_closest_turtle_first_:
                closests_turtle = None
                closests_turtle_distance = None
                ...
                for turtle in msg.turtles:
                    ...
                    if closests_turtle == None or distance < closests_turtle_distance:
                        closests_turtle = turtle
                        closests_turtle_distance = distance
                self.turtle_to_catch_ = closests_turtle
    ...
    def control_loop(self):
        ...
        if distance > 0.5:
            # Distance
            msg.linear.x = 2 * distance  # Simplifier P controller
    ...
    def callback_call_catch_turtle(self, future: Future, turtle_name):
        try:
